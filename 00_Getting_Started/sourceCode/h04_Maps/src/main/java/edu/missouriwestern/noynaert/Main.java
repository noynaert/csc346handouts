...
/**
 * This program is adapted from h03_inClass.
 *
 * It changes the implementation from an ArrayList to a Map.
 */
...
public class Main {
    public static void main(String[] args) {
        ...
        try {
            queryString = "select first_name, last_name, email from fakePeople where first_name like 'Ste%' and last_name like 'Mac%'  order by city";
            Map<String,Person> people = directQuery(credentials, queryString);
            people.forEach((e,k) -> System.out.println(k));
            //Map<String,Person> people = preparedQuery(credentials,queryString);
            //print(people,"People");
        }catch(Exception e){
    ...
    public static<E> void print(Map<String,Person> map, String message){
        System.out.printf("\n----- %s -----\n",message);
        for(String email:map.keySet()){
            System.out.println(map.get(email));
        }
        System.out.printf("----- %d items -----",map.size() );
    }

    private static Map<String,Person> preparedQuery(Credentials credentials, String queryString) throws SQLException, ClassNotFoundException {
...
//        ResultSet rs = stmt.executeQuery(queryString);
    ...
    private static Map<String,Person> directQuery(Credentials credentials, String queryString) throws SQLException, ClassNotFoundException {
        Map<String,Person> people = new HashMap<>();
